...
$query="update buses set rid=?,bname=?,bno=?,total_seat=?,source=?,destination=?,arrival=?,departure=?,price=?,status=? where id=?";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('sssissssiii',$rid,$bname,$bno,$seat,$source,$dest,$arrival,$dept,$price,$status,$id);
$stmt->execute();
	 ...
	 //$cnt=1;
	   while($row=$res->fetch_object())
	  {
	  	?>
					...
					<div class="col-sm-8">
					<input type="text"  name="rid" value="<?php echo $row->rid;?>"  class="form-control"> </div>
					</div>
