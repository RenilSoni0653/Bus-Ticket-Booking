...
{
	$op=base64_encode($_POST['oldpassword']);
	$np=base64_encode($_POST['newpassword']);
	$udate=date('d-m-Y h:i:s', time());
	
	$sql="SELECT password FROM user_reg where password=?";
	...
	if($row_cnt > 0)
	{
		$con="update user_reg set password=?,passUdateDate=? where id=?";
...
<?php $result ="SELECT passUdateDate FROM user_reg WHERE id=?";
